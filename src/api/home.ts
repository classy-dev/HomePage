...
export const fetchInstaFeed = async () => {
  const data = await AxiosUtil.get(
    `https://graph.instagram.com/me/media?fields=id,caption,media_url,timestamp,thumbnail_url,media_type,permalink&limit=3&access_token=${process.env.INSTAGRAM_KEY}`
  );
